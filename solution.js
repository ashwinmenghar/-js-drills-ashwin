...
const getEmails = () => arrayOfObjects.map((person) => person.email);

const getHobbiesByAge = (age) =>
  arrayOfObjects.filter((arr) => arr.age == age).map((arr) => arr.hobbies);

const getStudentsInAustralia = (isStudent, country) =>
  arrayOfObjects
    .filter((arr) => arr.isStudent == isStudent && arr.country == country)
    .map((arr) => arr.name);
...
const getNameAndCityAtIndex = (index) => {
  ...
  return { Name: obj.name, City: obj.city };
};

const printAgeOfEachPerson = () =>
  arrayOfObjects.forEach((arr) => console.log(arr.age));

const getFirstHobbyOfEachPerson = () =>
  arrayOfObjects.map((arr) => arr.hobbies[0]);

const getNamesAndEmailsByAge = (age) =>
  arrayOfObjects
